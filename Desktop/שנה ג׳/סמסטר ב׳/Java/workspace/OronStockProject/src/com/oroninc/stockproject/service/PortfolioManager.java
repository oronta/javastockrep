...
public class PortfolioManager implements PortfolioManagerInterface {
		...
		public void update() {
			...
			List<String> symbols = new ArrayList<>(Portfolio.getMaxSizePortfolio());
			for (StockInterface si : stocks) {
				symbols.add(si.getSymbol());
			}

			List<Stock> update = new ArrayList<>(Portfolio.getMaxSizePortfolio());
		...
		private PortfolioDto toDto(Portfolio portfolio) {
			...
			return new PortfolioDto(portfolio.getTitle(), (float) portfolio.getBalance(), array);
		}
